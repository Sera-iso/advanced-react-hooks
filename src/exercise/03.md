...
## 📝 Your Notes
Context allows to pass props down the component tree without having to prop drill. Because we can share the state between component without passing a prop, the components that dont use the context will not re-render, even if they are part of the same component tree. With props, we have to pass them down to the component tree, also to components that dont use them. As a consequence, if the state changes, we will have to re-render also the components that dont use those props, just because they are passed.
However context should not be used everwhere. Read below. 
