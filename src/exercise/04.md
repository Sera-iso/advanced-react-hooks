...
Anytime we are going to run effects that change the DOM in ways that the consumers can see, it's adviced using `useLayoutEffect`.
Otherwise we will see jumps in the layout because the browser paints the screen before the effect that change the DOM runs.  
